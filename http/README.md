# HTTP Internals and Building Your Own HTTP Client From Scratch Part 1
...
These methods are not obligatory, meaning that you can use a POST request to update something or use a GET request to delete something. As these methods are some kind of standards that we agree upon to improve readability. This means that you can use any kind of method to do whatever operation you want as long as you anc the backend side agreed to use this particular method for this particular operation.
...
The second part of the request line is the endpoint, which represents a specific resource or functionality provided by the server. You can think of it like the address where the server can find your required operation, you tell the server I want you to call this specific function written by the backend side and send me the results back, this function can be a database query, an image location, another HTTP request to another API written by the backend team, or just any other operation.
...
Sometimes you want to separate your request to several parts like sending mixed data types in a single request, uploading a file, uploading a file with a JSON objects, upload many files and many JSON objects, or you want to divide your requests to multiple parts for any reason. To achieve this you can construct a `MULTIPART form data` request. In this kind of requests, you can upload files (the most common use case for it) or divide your request into multiple parts for any reason. Unfortunately `MULTIPART` requests are not going to be discussed in this article to avoid lengthening this article.

## Building Your Own HTTP Client

Now that we learned about the construction of an HTTP request, let's build a program that constructs the HTTP request from scratch. Please note that this article assumes that you have a solid understanding of callbacks, generic data types, and socket programming. If you are not, please refer to these articles first in order to achieve solid understanding of this program:

1- [Callbacks](https://medium.com/@muhammad.heshamyt/callbacks-and-lambdas-in-java-vs-kotlin-675ed3495a65)
2- [Generics](https://medium.com/@muhammad.heshamyt/generics-wildcards-variance-and-star-projection-in-java-vs-kotlin-9553093485eb)
3- [Socket Programming](https://medium.com/@muhammad.heshamyt/introduction-to-raw-socket-programming-and-implementing-the-simplest-chat-service-java-17a014703f5f)

Now that you are ready, let's code!

Let's start by defining a callback that will be called once the reponse comes from the server either with success state or failure state:
...
```java
    ...
    public <T> void get(String endpoint, Callback<T> callback, Class<T> type) {
        ...
        try (BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
            ...
            // the request builder will contain out request details
...
Please note that I wrote this code only for illustration purposes, it's boilerplate and lacks a lot of refactoring to avoid duplicate code, testing, and more. It can be considered as a simple introdution to build your own HTTP client library, but an actual HTTP client library must contain more than just sending and receiving HTTP requests. A real HTTP clinet library should handle other HTTP methods, threading, serialization & deserialization, headers interception, request-retry mechanisms, cancellation strategies, logging, error handling, and really a lot more. If you are curious to explore more, you can give a visit to my full HTTP client library implementation from scratch from this [link](https://github.com/muhammadzkralla/ZHttp).
